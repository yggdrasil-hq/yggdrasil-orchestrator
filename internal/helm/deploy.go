...
// Deploy applies the placeholder chart to namespace as releaseName, using
// `helm upgrade --install` semantics (ADR 003 §13): installs the release if
// it doesn't exist yet, upgrades it in place otherwise. This mirrors the
...
func Deploy(ctx context.Context, cfg *action.Configuration, namespace, releaseName string, valueOverrides map[string]interface{}) error {
	chrt, err := loadPlaceholderChart()
	if err != nil {
		return err
	}
